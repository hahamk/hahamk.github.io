...
(function() {
	...
	LAYER_POPUP.prototype = {
		...
		show:function(element, url) {
			...
			that.layer.find('.layer-close').unbind('click').bind('click', $.proxy(that.hide, that));
...
$(function(){
	...
	$(document).on('click touchend','.bx-pager-link, .bxslider li',function() {
		player.stopAuto();
		player.startAuto();
	});
});
